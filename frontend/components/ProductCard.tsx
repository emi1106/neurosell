...
export default function ProductCard({ product, showFeatured }: ProductCardProps) {
  ...
  if (product.isFeatured && showFeatured) {
    return (
      <Link href={`/listings/${product.id}`} className="col-span-1 md:col-span-2 group cursor-pointer block">
        <div className="space-y-4">
  ...
  return (
    <Link href={`/listings/${product.id}`} className="group cursor-pointer block">
      {content}
    </Link>
  );
}
